...
Published on PyPI as `kore-memory` (v0.8.0). JS SDK on npm as `kore-memory-client` (v0.8.0). MIT license.
...
**Integrazioni (kore_memory/integrations/):**
- `langchain.py` — `KoreLangChainMemory` extending `BaseMemory` per LangChain chains
- `crewai.py` — `KoreCrewAIMemory` memory provider per CrewAI agents
- Entrambe opzionali: `pip install 'kore-memory[langchain]'` / `pip install 'kore-memory[crewai]'`
...
6 file in `tests/` — **165 test** totali. Usa `TestClient` FastAPI (in-process, no network). Ogni test crea un DB temporaneo (`KORE_DB_PATH` env var), usa `KORE_LOCAL_ONLY=1` per skip auth, isola con `X-Agent-Id: test-agent`.
...
- `test_mcp.py` — 32 test MCP server (14 tool)
- `test_langchain.py` — 28 test integrazione LangChain (mocked)
- `test_crewai.py` — 19 test integrazione CrewAI (mocked)

Config pytest in `pyproject.toml` (`asyncio_mode = "auto"`).

## CI/CD

- `.github/workflows/ci.yml` — push/PR su main: test (Python 3.11+3.12), test-semantic, security, lint, coverage (pytest-cov), test-js-sdk (Node 20)
